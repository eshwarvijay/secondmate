...
7. **Integrate.** Only after `verdict == pass` and a `PASS` gate and an answered hold. `scout` tasks stop at a
   ...
   is recovered automatically: release the lock, fetch, merge `origin/<base>` in with a distinctive
   `merge-sequencer: race recovery (attempt N/3)` commit, retry the push — bounded at 3 total attempts (the
   initial push plus 2 retries, never a 4th). `--preflight-only` runs `git merge-tree --write-tree` against a
