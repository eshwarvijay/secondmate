...
| `bin/claim-ledger.py` | Atomic task-id claims for concurrently-running sub-agent-supervisors, each in its own worktree — `claim`/`release`/`steal --reason`/`status`, append-only JSONL guarded by the same `fcntl`-based ledger lock as `hold.py`; `release` checks ownership, `steal` is a mandatory-reason human override that re-folds the ledger fresh inside the lock (no TOCTOU) |
...
| `SM_CLAIM_LEDGER` | `${SM_LOOP_STATE:-.secondmate}/claims.jsonl` | per-repo task-id claim ledger for `bin/claim-ledger.py` |
